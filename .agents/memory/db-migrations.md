...
**How to apply:** drizzle.config.ts uses relative schema/out paths — keep them relative (absolute paths break drizzle-kit generate). Before debugging widespread API test 500s after schema work, verify the development journal/table state and apply pending checked-in migrations. For prod schema issues, the answer is always "re-publish", never scripts/DDL against prod (prod SQL access is read-only anyway).

Development can also contain a schema object from an automatically reconciled merge while the matching migration journal row is absent. If `db:migrate` then fails because that exact object already exists, verify the object's definition matches the migration before stamping that one migration's exact SHA-256 and journal timestamp; never blindly skip a migration or use push.
