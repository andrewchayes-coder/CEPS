...
**How to apply:** Use the appropriate development sync path for schema work. A no-force Drizzle push can print "Changes applied" even when its verbose form lists unexpected drop/recreate statements; that success line is not evidence of a no-op. Inspect proposed SQL with strict confirmation before allowing a push when a clean diff is required. For production, let Publish apply the schema; plan data backfills as explicit one-time operations, reviewed separately from the deployment. Do not add automatic migration SQL to the deploy command.