...
Invoice and payment writes may only reference a vendor already associated with the participant through a non-deleted authorization, invoice, or payment. Authorization flows are the supported place to establish a new participant/vendor association. A referral-only vendor may be valid for a general participant vendor list, but is not yet eligible for an invoice or payment vendor selection.
...
**Why:** Browser-scoped pickers are not a security boundary. Crafted requests and concurrent soft deletes could otherwise create internally inconsistent participant records or leave partial financial side effects. The database's vendor-association guard does not count referrals; expanding only API validation would still fail at insert and mislead users.

**How to apply:** Reuse the shared participant-link validator for any create or edit path that accepts participant-dependent financial record IDs. Filter invoice vendor pickers to financially eligible links while keeping general vendor lists referral-inclusive. To support referral-only vendors in financial writes, update API and database guard together in an explicitly approved migration. Keep payment/fee and remittance/match/allocation writes in their existing atomic transactions. When adding a new link-bearing table, update both write validation and parent soft-delete guards, with tests for both race orderings.