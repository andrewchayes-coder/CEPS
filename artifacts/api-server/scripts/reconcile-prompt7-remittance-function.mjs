...
BEGIN
  ...
  SELECT (p.is_deleted = false AND p.client_id = expected_client_id AND (
    expected_authorization_id IS NULL OR p.authorization_id = expected_authorization_id OR EXISTS (
      SELECT 1 FROM payment_allocations pa WHERE pa.payment_id = p.id AND pa.authorization_id = expected_authorization_id
    )
  )) INTO valid_payment FROM payments p WHERE p.id = expected_payment_id FOR SHARE;
