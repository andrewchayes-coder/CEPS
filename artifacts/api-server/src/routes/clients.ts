...
function associatedWithVendor(vendorId: string): SQL {
  ...
  return sql`${clientsTable.id} in (
    select client_id
    from authorizations
    where authorizations.vendor_id = ${vendorId}
      and authorizations.is_deleted = false
    union
    select client_id
    from invoices
    where invoices.vendor_id = ${vendorId}
  ...
  if (query.data.search) {
    ...
    conditions.push(
      or(
        ...
        sql`${clientsTable.id} in (select client_id from invoices where invoices.is_deleted = false and (
