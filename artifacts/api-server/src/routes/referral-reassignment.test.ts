...
import { db, auditLogTable, clientsTable, referralsTable, sessionsTable, usersTable } from "@workspace/db";
import app from "../app";
...
beforeAll(async () => {
  const users = await db.insert(usersTable).values([
    { name: "Assignment Staff", email: `${nonce}-staff@test.local`, role: "staff" },
  ...
  [staffId, ownerId, activeCoordinatorId, inactiveCoordinatorId, nonCoordinatorId] = users.map((user) => user.id);
...
afterAll(async () => {
  ...
  await db.delete(sessionsTable).where(inArray(sessionsTable.userId, [staffId, ownerId, activeCoordinatorId]));
  await db.delete(auditLogTable).where(inArray(auditLogTable.userId, [staffId, ownerId]));
  await db.delete(usersTable).where(inArray(usersTable.id, [staffId, ownerId, activeCoordinatorId, inactiveCoordinatorId, nonCoordinatorId]));
});
