...
router.get("/vendors", requireAuth, async (req, res): Promise<void> => {
  ...
  if (query.data.clientId) {
    conditions.push(sql`${vendorsTable.id} in (
      select vendor_id
      from authorizations
      where authorizations.client_id = ${query.data.clientId}
        and authorizations.is_deleted = false
      union
      select vendor_id
      from invoices
      where invoices.client_id = ${query.data.clientId}
  ...
  if (query.data.search) {
    ...
    conditions.push(
      or(
        ...
        sql`${vendorsTable.id} in (select vendor_id from invoices where invoices.is_deleted = false and (
