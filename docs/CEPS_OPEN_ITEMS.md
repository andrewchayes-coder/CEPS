...
| 4 | ✅ Confirmed | **Fee auto-generation trigger rules** — the confirmed Alta rule is one active flat **$160 fee per participant and service month** whenever at least one non-deleted direct payment or reimbursement exists. Fee-type payments do not qualify. Payment creation, month/type changes, and deletion reconcile the month; only an untouched pending fee created by this rule is automatically reversed when the last qualifying payment is removed. Existing progressed or manually adjusted fees remain unchanged. Alta FMS historical imports do not generate fees. Staff also have read-only audit and explicitly confirmed repair endpoints for existing records; repair can be scoped to selected participants and leaves protected fees unchanged. | §5.2 step 6, Module 5, §12 | Confirmed flat monthly rule implemented |
...
| 7 | ✅ Confirmed | **Alta source layouts** — the Alta FMS Transaction Detail by Account workbook and Alta Payment History Detail Report CSV were supplied and used to finalize the payment and remittance import parsers. | §12 | Payments resolve by UCI/auth/month; remittances use the report summary plus detail sections |
