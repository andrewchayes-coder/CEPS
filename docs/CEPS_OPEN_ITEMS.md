...
| 4 | ✅ Confirmed | **Fee auto-generation trigger rules** — the confirmed Alta rule is one active flat **$160 fee per participant and service month** whenever at least one non-deleted direct payment or reimbursement exists. Fee-type payments do not qualify. Payment creation, month/type changes, and deletion reconcile the month; only an untouched pending fee created by this rule is automatically reversed when the last qualifying payment is removed. Existing progressed or manually adjusted fees remain unchanged. Alta FMS historical imports do not generate fees. | §5.2 step 6, Module 5, §12 | Confirmed flat monthly rule implemented |
