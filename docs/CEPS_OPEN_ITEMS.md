...
| 4 | 🟨 In progress | **Fee auto-generation trigger rules** — exact trigger conditions and qualifying service codes for auto-creating Fee records when a Payment is logged. **Done:** the `fees` entity now exists (`lib/db/src/schema/fees.ts`) with `GET/POST /fees` + `PATCH /fees/:id` and a client-detail Fees tab. Auto-generation runs for manual `POST /payments` entries, inserting a Fee of **5% of the payment amount**, `ruleApplied = interim_flat_percent_5_pending_confirmation`, status `pending`. Alta FMS historical imports do not generate fees. **Pending:** confirm real trigger conditions/qualifying service codes and amount rule with CEPS, then replace the interim placeholder in `payments.ts` (`autoGenerateFee`). | §5.2 step 6, Module 5, §12 | Interim 5% flat placeholder rule now live; swap once CEPS confirms |
